...
from evaluation import evaluate_index, evaluate_from_retriever, generate_qa_dataset, get_questions_from_nodes
...
def execute_dataset(args, dataset_name):
    ...
    query_engines = {
        ...
        'vector_kg_links': CustomRetriever(
            vector_index.as_retriever(),
            kg_links_index.as_retriever(),
        ),
        'vector_kw_table': CustomRetriever(
            vector_index.as_retriever(),
            kw_index.as_retriever(),
        ),
        'vector_kg': CustomRetriever(
            vector_index.as_retriever(),
    ...
    correctness_results = evaluate_from_retriever(
        req_nodes=req_nodes,
    ...
    eval_results = evaluate_index(questions, indices)

...
def get_docs_nxg(
    ...
    # # NetworkX draw function to plot the graph
    # nx.draw(docs_graph, with_labels=True, node_size=2000, node_color="skyblue", font_size=10)
    # plt.show()

   

    # # Save the graph as a PNG image
    # print("Saving the graph as an image...")
    # plt.figure(figsize=(12, 12))  # You can adjust the figure size as needed
    # nx.draw(docs_graph, with_labels=True, font_weight='bold', node_size=3000, node_color="skyblue", font_size=10, alpha=0.7)
    # plt.title("Codebase Class-Method Call Graph")
    # plt.savefig("class_method_call_graph.png", format="PNG")
    # print("Graph saved as 'class_method_call_graph.png'")
    
    # Optionally, you can display the graph if you are in an interactive environment
    # plt.show()  # Uncomment this if you're using an interactive environment


    # net = Network(height="800px", width="100%", notebook=True)

    # # Add nodes and edges from the NetworkX graph to Pyvis
    # for node in docs_graph.nodes:
    #     net.add_node(node, label=node, title=node)  # Add node

    # for edge in docs_graph.edges:
    #     net.add_edge(edge[0], edge[1])  # Add edge
    ...
    # # Add nodes for each class and method
    # for node in docs_graph.nodes:
    #     dot.node(node, label=node)  # Add node for class or method

    # # Add edges between nodes (class-method links)
    # for edge in docs_graph.edges:
    #     dot.edge(edge[0], edge[1])  # Add edge between class-method pairs

    # # Render and view the graph
    # print("Rendering graph...")
    # dot.render('graphviz_output', view=True)  # This will generate a PNG file and display it
...
if __name__ == '__main__':
    ...
    for dataset in ['iTrust']:
        ...
        # docs_graph, graph_nodes = get_docs_nxg(
        #     dataset_dir=f'data_repos/ftlr/datasets/{dataset}',
        #     all_code_files_path='all_code_filenames.txt',
        #     callgraph_file_name=f'{dataset.lower()}_method_callgraph.json',
        # )
